...
from donations.models import Donation, Fund

from .models import Category, CategoryKind, Pledge, Transaction
...
class FinanceViewTests(TestCase):
    ...
    def test_overview_combines_donations_and_transactions(self):
        ...
        resp = self.client.get(reverse("finance:overview") + "?year=2026")
    ...
    def test_expense_create_assigns_recorder_and_voucher(self):
        ...
        self.assertRedirects(resp, reverse("finance:transactions"))
    ...
    def test_ledger_does_not_leak_other_orgs(self):
        ...
        resp = self.client.get(reverse("finance:transactions"))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(list(resp.context["transactions"]), [])

    def test_budgets_and_pledges_pages_render(self):
        self.assertEqual(self.client.get(reverse("finance:budgets")).status_code, 200)
        self.assertEqual(self.client.get(reverse("finance:pledges")).status_code, 200)
        self.assertEqual(self.client.get(reverse("finance:categories")).status_code, 200)

    def test_add_category_via_post(self):
        resp = self.client.post(
            reverse("finance:categories"),
            {"kind": "income", "name": "Donations Box", "description": ""},
        )
        self.assertRedirects(resp, reverse("finance:categories"))
        self.assertTrue(
